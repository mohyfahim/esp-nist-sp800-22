                     ...
                     int bitArrayLen)
{
	...
	ESP_LOGI(TAG, "\t\t\t  RANDOM EXCURSIONS TEST\n");
	ESP_LOGI(TAG, "\t\t--------------------------------------------\n");
#endif
    ...
    {
#ifdef INTERNAL_DEBUG
		ESP_LOGI(TAG, "Error: Insufficent Work Space Allocated.\n");
#endif
		if ( S_k != NULL )
		{
			free(S_k);
	...
	for( i = 1; i < bitArrayLen; i++ )
	{
		...
		if ( S_k[i] == 0 )
		{
			...
			{
#ifdef INTERNAL_DEBUG
				ESP_LOGI(TAG, "Error: EXCEEDING THE MAX NUMBER OF CYCLES EXPECTED\n.");
#endif
				free(S_k);
				free(cycle);
...
#ifdef INTERNAL_DEBUG
		ESP_LOGI(TAG, "%s\t\tx = %2d chi^2 = %9.6f p_value = %f\n",
