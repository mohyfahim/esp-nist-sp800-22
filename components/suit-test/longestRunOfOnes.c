...
int LongestRunOfOnes(const unsigned char *bitArray, int bitArrayLen)
{
	...
	ESP_LOGI(TAG, "\t\t\t  LONGEST RUNS OF ONES TEST\n");
	ESP_LOGI(TAG, "\t\t---------------------------------------------\n");
#endif
	...
	ESP_LOGI(TAG, "\t\t---------------------------------------------\n");
	ESP_LOGI(TAG, "\t\t(a) N (# of substrings)  = %d\n", blockCount);
	ESP_LOGI(TAG, "\t\t(b) M (Substring Length) = %d\n", blockLen);
	...
	{
#ifdef INTERNAL_DEBUG
		ESP_LOGI(TAG, "\t\t  <=1     2     3    >=4   P-value  Assignment");
	...
	else
	{
		...
		{
#ifdef INTERNAL_DEBUG
			ESP_LOGI(TAG, "\t\t<=4  5  6  7  8  >=9 P-value  Assignment");
		...
		{
#ifdef INTERNAL_DEBUG
			ESP_LOGI(TAG,"\t\t<=10  11  12  13  14  15 >=16 P-value  Assignment");
