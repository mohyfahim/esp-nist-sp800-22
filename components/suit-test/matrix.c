...
unsigned char** create_matrix(int M, int Q)
{
	...
	if ( (matrix = (unsigned char **)calloc(M, sizeof(unsigned char *))) == NULL )
	{
		...
		ESP_LOGI(TAG,"ERROR IN FUNCTION create_matrix:  Insufficient memory available.\n");
#endif
		return NULL;
	}
	...
	for ( i = 0; i < M; i++ )
	{
		...
		{
#ifdef INTERNAL_DEBUG
			ESP_LOGI(TAG,"ERROR IN FUNCTION create_matrix: Insufficient memory for %dx%d matrix.\n", M, M);
#endif
			return NULL;
		}
	}
