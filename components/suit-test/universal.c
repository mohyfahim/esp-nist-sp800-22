...
int Universal(const unsigned char *bitArray, int bitArrayLen)
{
	...
	ESP_LOGI(TAG, "\t\t\tUNIVERSAL STATISTICAL TEST\n");
	ESP_LOGI(TAG, "\t\t--------------------------------------------\n");
	...
	ESP_LOGI(TAG, "\t\t(b) Q         = %d\n", Q);
	ESP_LOGI(TAG, "\t\t(c) K         = %d\n", K);
	ESP_LOGI(TAG, "\t\t(d) sum       = %f\n", sum);
	ESP_LOGI(TAG, "\t\t(e) sigma     = %f\n", sigma);
	ESP_LOGI(TAG, "\t\t(f) variance  = %f\n", variance[blockLen]);
	ESP_LOGI(TAG, "\t\t(g) exp_value = %f\n", expected_value[blockLen]);
	ESP_LOGI(TAG, "\t\t(h) phi       = %f\n", phi);
	ESP_LOGI(TAG, "\t\t(i) WARNING:  %d bits were discarded.\n", bitArrayLen - (Q + K) * blockLen);
	ESP_LOGI(TAG, "\t\t-----------------------------------------\n");
#endif
